...
# Write-Information is used in place of Write-Host so the output can be captured, redirected
# or suppressed; $InformationPreference keeps it visible by default, as Write-Host was.
...
if ($runExitCode -ne 0) {
	Write-Information ""
	Write-Information "$($PSStyle.Foreground.Red)The release run did not succeed: https://github.com/$repoFullName/actions/runs/$runId$($PSStyle.Reset)"
	...
	if ($LASTEXITCODE -eq 0 -and $jobId) {
		...
		if ($LASTEXITCODE -eq 0 -and $annotation) {
			Write-Information "$($PSStyle.Foreground.Red)Reason: $annotation$($PSStyle.Reset)"
		}
	}

	Write-Information ""
	Write-Information "$($PSStyle.Foreground.Yellow)Tag $version is pushed but no package was published. Once the cause is fixed:$($PSStyle.Reset)"
	Write-Information "$($PSStyle.Foreground.Cyan)  gh run rerun $runId --repo $repoFullName --failed$($PSStyle.Reset)"
	exit 1
}

Write-Information "$($PSStyle.Foreground.Green)Package $version published.$($PSStyle.Reset)"